...
namespace Chraft.Net
{
	...
	{
		public NetworkStream Net { get; private set; }
		...
		public BigEndianStream(NetworkStream stream)
		{
			Net = stream;
		}
		...
		public string ReadString(short maxLen)
		{
			int len = ReadShort();
			if (len > maxLen)
				throw new Exception("String field too long");
			byte[] b = new byte[len];
			for (int i = 0; i < len; i++)
				b[i] = (byte)ReadByte();
			return ASCIIEncoding.UTF8.GetString(b);
