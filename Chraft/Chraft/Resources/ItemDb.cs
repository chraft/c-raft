...
namespace Chraft.Resources
{
	public class ItemDb
	{
		...
		public ItemStackChraft this[string item]
		{
			get
			{
				if (Contains(item))
					return new ItemStackChraft(Items[item], Settings.Default.DefaultStackSize, Durabilities[item]);
				else
					return ItemStackChraft.Void;
			}
		}
