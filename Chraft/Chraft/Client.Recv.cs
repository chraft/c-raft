...
namespace Chraft
{
    public partial class Client
    {
		...
		{
			PlayerAnimationEvent ev = new PlayerAnimationEvent(this);
			Server.BukkitPluginManager.callEvent(ev);
        ...
        private void PacketHandler_UseBed(object sender, PacketEventArgs<UseBedPacket> e)
        {
			if (e.Packet.InBed == 0)
			{
				PlayerBedLeaveEvent ev = new PlayerBedLeaveEvent(this, World.getBlockAt(e.Packet.X, e.Packet.Y, e.Packet.Z));
				Server.BukkitPluginManager.callEvent(ev);
			}
			else
			{
				PlayerBedEnterEvent ev = new PlayerBedEnterEvent(this, World.getBlockAt(e.Packet.X, e.Packet.Y, e.Packet.Z));
				Server.BukkitPluginManager.callEvent(ev);
			}
        }
        ...
        private void PacketHandler_PlayerRotation(object sender, PacketEventArgs<PlayerRotationPacket> e)
        {
			...
			this.UpdateEntities();
			UpdateOnGround(e.Packet.OnGround);
        }
